...
def test_re_findall_groupdict():
    assert Parsel().xpath("//body/h1/text()").get().re_findall(r"(?P<word>\w+)", groupdict=True).sc_parse(
        HTML
    ) == [{"word": "Hello"}, {"word": "Parsel"}]


def test_re_findall_groupdict_fail():
    with pytest.raises(TypeError):
        Parsel().xpath("//body/h1/text()").get().re_findall(r"(\w+)", groupdict=True).sc_parse(
            HTML)
