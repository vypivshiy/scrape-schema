...
def test_getall():
    assert Parsel().xpath("//li/a/@href").getall().sc_parse(HTML) == [
        "http://example.com",
        "http://scrapy.org",
    ]
...
def test_jmespath():
    assert Parsel().css("script::text").jmespath("a").getall().sc_parse(HTML) == [
        "b",
        "c",
    ]
...
def test_incorrect_type():
    with pytest.raises(TypeError):
        Parsel().xpath("//li/a").sc_parse(0)


def test_parse_from_bytes():
    assert Parsel().xpath("//li/a/@href").getall().sc_parse(HTML.encode()) == [
        "http://example.com",
        "http://scrapy.org",
    ]
