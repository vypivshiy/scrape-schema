...
class Book(MainSchema):
    ...
    _url_path: ScField[
        str, ParselSelect("div.image_container > a", callback=get_attr("href"))
    ]
    _image_path: ScField[
        str, ParselSelect("div.image_container > a > img", callback=get_attr("src"))
    ]
    _rating: ScField[
        str,
        ParselSelect(
            "p.star-rating", callback=lambda css: css.attrib.get("class").split()[-1]
        ),
    ]
    ...
    available: ScField[
        str,
        ParselSelect(
            "div.product_price > p.instock.availability",
