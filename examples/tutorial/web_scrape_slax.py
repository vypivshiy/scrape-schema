...
class Book(MainSchema):
    ...
    _url_path: ScField[
        str, SlaxSelect("div.image_container > a", callback=get_attr("href"))
    ]
    _image_path: ScField[
        str, SlaxSelect("div.image_container > a > img", callback=get_attr("src"))
    ]
    _rating: ScField[
        str,
        SlaxSelect(
            "p.star-rating", callback=lambda node: node.attrs.get("class").split()[-1]
        ),
    ]
...
class CataloguePage(MainSchema):
    books: ScField[
        List[Book],
        NestedList(Book, crop_rule=crop_by_slax_all("section > div > ol.row > li")),
    ]
