...
from scrape_schema.callbacks.soup import get_tag, replace_text
...
class Post(BaseSchema):
    ...
    id: Annotated[int, SoupFind('<tr class="athing">', callback=get_tag('id'))]
    ...
    vote_up: Annotated[str, SoupFind({'name': 'a', 'id': re.compile(r'^up_\d+')},
                                     callback=get_tag('href'), factory=concat_href)]
    source: Annotated[str, SoupSelect('td.title > span.titleline > a', callback=get_tag('href'))]
    ...
    author_url: Annotated[str, SoupFind('<a class="hnuser">', callback=get_tag('href'), factory=concat_href)]
    date: Annotated[str, SoupFind('<span class="age">', callback=get_tag('title'))]
    time_ago: Annotated[str, SoupSelect('span.age > a')]
    comments: Annotated[int, ReMatch(r'(\d+)\s+comments', default=0)]
    post_url: Annotated[str, SoupFind({'name': 'a', 'href': re.compile(r'^item\?id=\d+')}, callback=get_tag('href'),
                                      factory=concat_href)]
