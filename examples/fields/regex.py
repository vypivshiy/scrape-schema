...
class Schema(BaseSchema):
    ...
    digit_float_x35: Annotated[float, ReMatch(r'(\d)', callback=float, factory=lambda s: s+3.5)]
    ...
    sum_less_3: Annotated[int, ReMatchList(r'(\d)', filter_=lambda s: int(s) <= 3, callback=int, factory=sum)]
    # get digits, then bigger than 3, convert to int
    bigger_3: Annotated[list[int], ReMatchList(r'(\d)', filter_=lambda s: int(s) > 3, callback=int)]
    # get digits, then bigger than 3 and sum
    sum_bigger_3: Annotated[int, ReMatchList(r'(\d)', filter_=lambda s: int(s) > 3, callback=int, factory=sum)]
